...
public class PdfService {
    ...
    private void addSeccionEmpresa(Document document) {
        ...
        Cell datosFiscalesEmpresa = new Cell().setBorder(Border.NO_BORDER).setTextAlignment(TextAlignment.RIGHT);
    ...
    private void addSeccionCliente(Document document, FacturaConId factura) {
        ...
        tablaDatos.addCell(
                new Cell().add(new Paragraph("Factura Nº").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
        tablaDatos.addCell(new Cell().add(new Paragraph("2025/" + String.valueOf(factura.getId()))));

        tablaDatos.addCell(
                new Cell().add(new Paragraph("Concepto").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
        tablaDatos.addCell(new Cell().add(new Paragraph(factura.getConceptoFactura())));

        tablaDatos.addCell(
                new Cell().add(new Paragraph("Cliente").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
        tablaDatos.addCell(new Cell().add(new Paragraph(factura.getNombreApellidosDNI())));

        tablaDatos.addCell(
                new Cell().add(new Paragraph("Dirección").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
        tablaDatos.addCell(new Cell().add(new Paragraph(factura.getDatosDireccionLocalizacion())));

        tablaDatos.addCell(
                new Cell().add(new Paragraph("Vehículo").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
    ...
    private void addSeccionDatosVenta(Document document, FacturaConId factura) {
        ...
        tablaImportes.addCell(
                new Cell().add(new Paragraph("Importe base").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
        tablaImportes.addCell(new Cell().add(new Paragraph(formatNumero(factura.getImporte()) + " €" ))
                .setTextAlignment(TextAlignment.RIGHT));

        float totalImpuestos = factura.getImporte() * 0.21f;
        tablaImportes.addCell(
                new Cell().add(new Paragraph("Impuestos (I.V.A. 21%)").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
    ...
    private void addSeccionDatosAlquiler(Document document, FacturaConId factura) {
        ...
        tablaImportes.addCell(
                new Cell().add(new Paragraph("Fecha de inicio del alquiler").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
        tablaImportes.addCell(new Cell().add(new Paragraph(factura.getFechaInicioAlquiler().format(formatter)))
                .setTextAlignment(TextAlignment.RIGHT));

        tablaImportes.addCell(
                new Cell().add(new Paragraph("Fecha de fin del alquiler").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
        tablaImportes.addCell(new Cell().add(new Paragraph(factura.getFechaFinAlquiler().format(formatter)))
                .setTextAlignment(TextAlignment.RIGHT));

        tablaImportes.addCell(
                new Cell().add(new Paragraph("Total importe de días del alquiler (" + factura.getNumeroDiasAlquiler() +", " + factura.getImporte() +"€/dia)" ).setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
        tablaImportes.addCell(new Cell().add(new Paragraph(formatNumero(factura.getImporteTotalDias()) + " €" ))
                .setTextAlignment(TextAlignment.RIGHT));

        tablaImportes.addCell(
                new Cell().add(new Paragraph("Total importe por KMs extra").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
        tablaImportes.addCell(new Cell().add(new Paragraph(formatNumero(factura.getImporteKmExtra()) + " €" ))
                .setTextAlignment(TextAlignment.RIGHT));

        tablaImportes.addCell(
                new Cell().add(new Paragraph("Total importe penalización depósito").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
        tablaImportes.addCell(new Cell().add(new Paragraph(formatNumero(factura.getImportePenalizacionDeposito()) + " €" ))
                .setTextAlignment(TextAlignment.RIGHT));

        float importeBase = factura.getImporteTotalDias() + factura.getImporteKmExtra() + factura.getImportePenalizacionDeposito();
        tablaImportes.addCell(
                new Cell().add(new Paragraph("Importe base").setBold()).setBackgroundColor(ColorConstants.LIGHT_GRAY));
    ...
    private void addInformacionPago(Document document, String id) {
        ...
        Paragraph texto = new Paragraph("Por favor, realice el pago a través de los siguientes datos bancarios:")
                .setFontSize(10)
                .setTextAlignment(TextAlignment.LEFT)
                .setMarginBottom(5);
        document.add(texto);
        ...
        tablaPago.addCell(new Cell().add(new Paragraph("ES12 3456 7890 1234 5678 9012")).setBorder(Border.NO_BORDER));
    ...
    private void addAgradecimiento(Document document) {
        ...
        document.add(new Paragraph("Gracias por confiar en Alvencar S.L.U. para la compra y alquiler de vehículos.")
